...
class Patient:
    ...
    def __init__(self, name, age, diagnosis, doctor_specialization, temperature=36.6, systolic=120, diastolic=80):
        self._validate_all(name, age, diagnosis, doctor_specialization, temperature, systolic, diastolic)
    ...
    def _validate_all(self, name, age, diagnosis, doctor_specialization, temperature, systolic, diastolic):
        ...
        doctor_valid, doctor_msg = validate.validate_doctor_specialization(doctor_specialization)
        if not doctor_valid:
        ...
        pressure_valid, pressure_msg = validate.validate_pressure(systolic, diastolic)
    ...
    def diagnosis(self, new_diagnosis):
        ...
        status_valid, status_msg = validate.validate_status_for_operation(self.__status, "изменить диагноз")
        if not status_valid:
            raise ValueError(status_msg)
        
        old_diagnosis = self.__diagnosis
        self.__diagnosis = new_diagnosis.strip()
        self._add_to_history(f"Диагноз изменен с '{old_diagnosis}' на '{self.__diagnosis}'")
    ...
    def doctor_specialization(self, new_specialization):
        ...
        self._add_to_history(f"Специализация врача изменена с '{old_spec}' на '{self.__doctor_specialization}'")
    ...
    def temperature(self, new_temperature):
        ...
        self._add_to_history(f"Температура изменена с {old_temp}°C на {self.__temperature}°C")

    def update_vitals(self, temperature=None, systolic=None, diastolic=None):
        status_valid, status_msg = validate.validate_status_for_operation(self.__status, "обновить показатели")
        if not status_valid:
            raise ValueError(status_msg)
        ...
        if systolic is not None or diastolic is not None:
            ...
            if systolic is not None:
                ...
                self.__systolic = int(systolic)
                changes.append(f"систолическое: {old_systolic} → {self.__systolic}")
            
            if diastolic is not None:
                old_diastolic = self.__diastolic
                self.__diastolic = int(diastolic)
                changes.append(f"диастолическое: {old_diastolic} → {self.__diastolic}")
    ...
    def discharge(self):
        ...
        needs_hospital, msg = validate.validate_hospitalization(self.__age, self.__diagnosis)
        if needs_hospital:
            raise ValueError(f"Нельзя выписать: {msg}")
        
        self.__discharge_date = datetime.now()
        self._update_status("выписан")
        self._add_to_history(f"Пациент выписан. Дней в больнице: {self.days_in_hospital}")
    ...
    def mark_as_deceased(self):
        ...
        self._add_to_history(f"Пациент умер. Дней в больнице: {self.days_in_hospital}")
    
    def transfer_to_doctor(self, new_specialization):
        status_valid, status_msg = validate.validate_status_for_operation(self.__status, "перевести к другому врачу")
        if not status_valid:
            raise ValueError(status_msg)
        ...
        old_spec = self.__doctor_specialization
        self.__doctor_specialization = new_specialization.strip().lower()
        self._add_to_history(f"Переведен к врачу: {old_spec} → {self.__doctor_specialization}")
    
    def add_treatment_note(self, note):
        status_valid, status_msg = validate.validate_status_for_operation(self.__status, "добавить заметку")
        if not status_valid:
            raise ValueError(status_msg)
...
╚════════════════════════════════════════════════════════════╝"""
    ...
    def __repr__(self):
        return (f"Patient(name='{self.__name}', age={self.__age}, "
