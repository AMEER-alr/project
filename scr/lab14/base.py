...
class Patient(Printable, Comparable, MedicalInfo):
    ...
    def to_string(self) -> str:
        return f"[{self.get_patient_type()}] {self.name}, {self.age} лет, {self.diagnosis}"
    ...
    def get_medical_summary(self) -> str:
        return f"ID: {self.patient_id} | {self.name} | Диагноз: {self.diagnosis} | Статус: {self.status}"
