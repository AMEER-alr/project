...
class PatientCollection:
    ...
    def add(self, item):
        patient_types = (Patient, InPatient, OutPatient, EmergencyPatient)
        if not isinstance(item, patient_types):
            raise TypeError(f"Можно добавлять только объекты Patient. Получен: {type(item)}")
